...
{
  float g2 = g * g;
  return (1.0f - g2) / pow(1.0f + g2 - 2.0f * g * saturate(theta), 1.5f) / (4.0f * std::numbers::pi_v<float>);
}
...
float noise(float2 uv)
{
  ...
  return fract(dot(sin(a * b * 1024.0f), float3(341896.483, 891618.637, 602649.7031)));
}
} // namespace
