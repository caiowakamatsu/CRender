...
namespace cr {
	...
	public:
		scene_configuration(const glm::vec3 &origin, const glm::vec3 rotation, uint64_t width, uint64_t height, float fov,
												uint64_t bounces)
						: _pv_matrix(1.0f), _width(width), _height(height), _rotation(rotation), _fov(fov), _bounces(bounces), _origin(origin) {
		...
		[[nodiscard]] glm::mat4 rotation_matrix() const noexcept {
			...
			rotation_matrix = glm::rotate(rotation_matrix, glm::radians(_rotation.x), UP);
			rotation_matrix = glm::rotate(rotation_matrix, glm::radians(_rotation.y), RIGHT);
			rotation_matrix = glm::rotate(rotation_matrix, glm::radians(_rotation.z), FORWARD);
		...
		[[nodiscard]] cr::ray ray(uint64_t x, uint64_t y, cr::random &rand) const noexcept {
			const auto u = (((static_cast<float>(x) +  rand.next_float()) / _width) * 2.0f - 1.0f) * aspect();
			const auto v = ((static_cast<float>(y) + rand.next_float()) / _height) * 2.0f - 1.0f;
