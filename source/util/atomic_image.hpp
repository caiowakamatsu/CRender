...
public:
  ...
  atomic_image(uint64_t width, uint64_t height)
      : _width(width), _height(height) {
    auto float_zero_as_uint32 = ::float_to_uint32(0.0f);
    _image_data =
        std::vector<uint32_t>(_width * _height * 4, float_zero_as_uint32);
  }
  ...
  [[nodiscard]] glm::vec4 operator[](size_t index) const noexcept {
    return {::load_uint32_atomic_as_float(_image_data[index + 0]),
