...
namespace glm {
	...
	GLM_FUNC_QUALIFIER void compose(glm::mat<4, 4, T, Q>& ModelMatrix, const glm::vec<3, T, Q>& Scale, const glm::qua<T, Q>& Orientation, const glm::vec<3, T, Q>& Translation)
	{
	...
	template<typename T, glm::qualifier Q>
	GLM_FUNC_DECL glm::mat<4, 4, T, Q> compose(glm::vec<3, T, Q> const& Scale, glm::qua<T, Q> const& Orientation, glm::vec<3, T, Q> const& Translation)
	{
...
namespace cr::gltf {
	...
	[[nodiscard]] loaded_node get_vertices(const tinygltf::Model &model, const tinygltf::Node &node, const glm::mat4 &transform) {
		...
		for (const auto &primitive : mesh.primitives)
		{
			auto indices = std::vector<uint16_t>();
			{
				...
				const auto stride = view.byteStride == 0 ? sizeof(uint16_t) : view.byteStride; // Is this correct?
				...
				for (size_t i = 0; i < accessor.count; i++)
				{
					...
					std::memcpy(&number, buffer.data.data() + view.byteOffset + offset, sizeof(uint16_t));
					indices[i] = number;
				}
			}
			...
			auto vertices = std::vector<glm::vec3>();
			{
				...
				const auto stride = view.byteStride == 0 ? sizeof(glm::vec3) : view.byteStride;
				for (size_t i = 0; i < accessor.count; i++)
				{
					const auto offset = stride * i;

					auto vertex = glm::vec3();
					std::memcpy(&vertex, buffer.data.data() + view.byteOffset + offset, sizeof(glm::vec3));
					vertices[i] = vertex;
				}
			}
			...
			auto texcoords = std::vector<glm::vec2>();
			{
				...
				const auto stride = view.byteStride == 0 ? sizeof(glm::vec2) : view.byteStride;
				for (size_t i = 0; i < accessor.count; i++)
				{
					const auto offset = stride * i;

					auto texcoord = glm::vec2();
					std::memcpy(&texcoord, buffer.data.data() + view.byteOffset + offset, sizeof(glm::vec2));
			...
			auto normals = std::vector<glm::vec3>();
			{
				...
				for (size_t i = 0; i < accessor.count; i++)
				{
					...
					std::memcpy(&normal, buffer.data.data() + view.byteOffset + offset, sizeof(glm::vec3));
					normals[i] = normal;
				}
			}
	...
	[[nodiscard]] glm::mat4 get_transformation(const tinygltf::Node &node) {
		...
		if (auto &mat = node.matrix; mat.size() == 16)
			transform = {mat[0], mat[1], mat[2], mat[3],
	...
	[[nodiscard]] std::optional<glm::vec3> material_albedo(tinygltf::ParameterMap *additionalValues) {
		...
		// Todo: add more ways for checking base colour (are there any other conventions?)
	...
	[[nodiscard]] std::optional<cr::image> material_albedo_texture(
					...
					std::span<tinygltf::Sampler> samplers) {

		if (material->pbrMetallicRoughness.baseColorTexture.index != -1) {
			const auto texture = textures[material->pbrMetallicRoughness.baseColorTexture.index];
	...
	[[nodiscard]] std::optional<cr::image> material_emissive_texture(
		...
		if (material->emissiveTexture.index != -1) {
			...
			const auto image = images[texture.source];
			// Todo: use sampler somehow...?
//			[[maybe_unused]] const auto sampler = samplers[texture.sampler];
	...
	[[nodiscard]] std::optional<glm::vec3> material_emission(const std::vector<double> &emissive) {
		if (emissive.size() == 3) {
	...
	[[nodiscard]] std::optional<float> material_roughness(tinygltf::Material *material) {
	...
	[[nodiscard]] std::optional<cr::image> material_roughness_texture(
		...
		if (material->pbrMetallicRoughness.metallicRoughnessTexture.index != -1) {
			const auto texture = textures[material->pbrMetallicRoughness.metallicRoughnessTexture.index];
			const auto image = images[texture.source];
			// Todo: use sampler somehow...?
			if (image.mimeType.find("png") != std::string::npos) {
				if (image.component != 4) {
	...
	[[nodiscard]] std::optional<float> material_metalness(tinygltf::Material *material) {
		auto values = material->values;
	...
	[[nodiscard]] std::unique_ptr<cr::material> load_material(
		...
		const auto roughness_texture = material_roughness_texture(material, textures, images, samplers);
		...
		const auto metalness_texture = material_metalness_texture(material, textures, images, samplers);
		...
		const auto emissive_texture = material_emissive_texture(material, textures, images, samplers);
		const auto emission = [&](){
		...
		const auto base_colour_texture = material_albedo_texture(material, textures, images, samplers);
