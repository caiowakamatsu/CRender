...
cr::display::display(size_t width, size_t height) : _width(width), _height(height) {
	if (!glfwInit()) {
...
void cr::display::render(render_data data) {
	...
	[[maybe_unused]] const auto options = cr::component::interface<cr::component::preview>().display({
		.frame = data.frame,
	});

	[[maybe_unused]] const auto console = cr::component::interface<cr::component::console>().display({
		.lines = data.lines
	});

	[[maybe_unused]] const auto settings = cr::component::interface<cr::component::settings>().display({});
...
void cr::display::_glfw_cursor_position_callback(GLFWwindow *window, double x, double y) {
	auto user_window = static_cast<cr::display *>(glfwGetWindowUserPointer(window));
	user_window->_mouse.set_position({x, y});
}

void cr::display::_glfw_mouse_button_callback(GLFWwindow *window, int button, int action, int mods) {
	auto user_window = static_cast<cr::display *>(glfwGetWindowUserPointer(window));
...
void cr::display::_glfw_key_callback(GLFWwindow *window, int key, int scancode, int action, int mods) {
	auto user_window = static_cast<cr::display *>(glfwGetWindowUserPointer(window));

	if (action == GLFW_RELEASE)
		user_window->_keyboard.keys()[key].press_state = cr::keyboard::state::released;
	else if (action == GLFW_REPEAT)
		user_window->_keyboard.keys()[key].press_state = cr::keyboard::state::repeat;
	else if (action == GLFW_PRESS)
		user_window->_keyboard.keys()[key].press_state = cr::keyboard::state::pressed;
