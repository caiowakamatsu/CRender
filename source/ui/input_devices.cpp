...
cr::keyboard::state cr::keyboard::state_of(cr::keyboard::key_code key) const noexcept {
...
std::array<cr::keyboard::key_state, static_cast<size_t>(cr::keyboard::key_code::max_key)> &
cr::keyboard::keys() {
	return _keys;
}
...
void cr::mouse::set_position(const glm::ivec2 &pos) {
	_position = pos;
}
...
cr::mouse::button_state cr::mouse::state_of(cr::mouse::button target) const noexcept {
	return _buttons[static_cast<int>(target)];
}

glm::ivec2 cr::mouse::position() const noexcept {
	return _position;
}
...
bool cr::mouse::hidden() const noexcept {
	return _hidden;
}
