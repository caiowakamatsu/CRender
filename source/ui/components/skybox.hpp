...
ComputeWaveLambdaRayleigh(const glm::vec3 &lambda) {
  const float n = 1.0003f;
  const float N = 2.545E25;
  const float pn = 0.035f;
  const float n2 = n * n;
  const float pi3 = std::numbers::pi_v<float> * std::numbers::pi_v<float> *
                    std::numbers::pi_v<float>;
  const float rayleighConst = (8.0f * pi3 * pow(n2 - 1.0f, 2.0f)) / (3.0f * N) *
...
public:
  struct Options {
    ...
    glm::vec3 waveLambdaOzone =
        glm::vec3(1.36820899679147, 3.31405330400124, 0.13601728252538) *
        0.6e-6f * 2.504f;
    glm::vec3 waveLambdaRayleigh =
        ComputeWaveLambdaRayleigh({680e-9, 550e-9, 450e-9});
  };
