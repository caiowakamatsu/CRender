...
glm::vec3 cr::brdf::gltf_brdf(const glm::vec3 &V, const glm::vec3 &L, const glm::vec3 &N, const glm::vec3 &H,
															const glm::vec3 &base_colour, const float &roughness, const float &metalness) noexcept {
	...
	const auto F = f0 + (1.0f - f0) * glm::pow(1.0f - glm::abs(glm::dot(V, H)), 5.0f);
...
float cr::brdf::specular_brdf(const glm::vec3 &N, const glm::vec3 &L, const glm::vec3 &V, const glm::vec3 &H, float a) {
	const auto d = brdf::ggx_microfacet(N, H, a);
	const auto v = brdf::visibility(H, V, L, N, a);
	return d * v;
}

float cr::brdf::ggx_microfacet(const glm::vec3 &N, const glm::vec3 &H, float a) {
	const auto a2 = a * a;
...
float cr::brdf::visibility(const glm::vec3 &H, const glm::vec3 &V, const glm::vec3 &L, const glm::vec3 &N, float a) {
