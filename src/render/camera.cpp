...
cr::camera::camera(const glm::vec3 position, const glm::vec3 look_at, float fov, float aspect)
{
...
cr::ray cr::camera::get_ray(float x, float y) const noexcept
{
    const auto x_offset  = horizontal * (x * 2.f - 1);
    const auto y_offset  = vertical * (y * 2.f - 1);
    const auto direction = glm::normalize((center + x_offset + y_offset) - position);
