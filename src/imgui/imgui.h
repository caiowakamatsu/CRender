...
struct ImGuiPlatformMonitor;        // Multi-viewport support: user-provided bounds for each connected monitor/ui. Used when positioning popups and tooltips to avoid them straddling monitors
...
// Character types
// (we generally use UTF-8 encoded string in the API. This is storage specifically for a decoded character used for keyboard input and ui)
...
namespace ImGui
{
    ...
    IMGUI_API void          ShowMetricsWindow(bool* p_open = NULL);     // create Metrics/Debugger window. ui Dear ImGui internals: windows, draw commands, various internal state, etc.
    IMGUI_API void          ShowAboutWindow(bool* p_open = NULL);       // create About window. ui Dear ImGui version, credits and build/system information.
    ...
    IMGUI_API void          LabelText(const char* label, const char* fmt, ...)              IM_FMTARGS(2); // ui text+label aligned the same way as value+label widgets
    ...
    // - Adjust format string to decorate the value with a prefix, a suffix, or adapt the editing and ui precision e.g. "%.3f" -> 1.234; "%5.2f secs" -> 01.23 secs; "Biscuit: %.0f" -> Biscuit: 1; etc.
    ...
    //   If you get a warning converting a float to ImGuiSliderFlags, read https://github.com/ocornut/imgui/issues/3361
    IMGUI_API bool          SliderFloat(const char* label, float* v, float v_min, float v_max, const char* format = "%.3f", ImGuiSliderFlags flags = 0);     // adjust format to decorate the value with a prefix or a suffix for in-slider labels or unit ui.
    ...
    IMGUI_API bool          ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags = 0, ImVec2 size = ImVec2(0, 0)); // ui a color square/button, hover for details, return true when pressed.
    ...
    IMGUI_API bool          CollapsingHeader(const char* label, bool* p_visible, ImGuiTreeNodeFlags flags = 0); // when 'p_visible != NULL': if '*p_visible==true' ui an additional small close button on upper right of the header which will set the bool to false when clicked, if '*p_visible==false' don't ui the header.
    IMGUI_API void          SetNextItemOpen(bool is_open, ImGuiCond cond = 0);                  // set next TreeNode/CollapsingHeader open state.

    // Widgets: Selectables
    // - A selectable highlights when hovered, and can ui another color when selected.
    ...
    IMGUI_API void          LogButtons();                                                       // helper to ui buttons for logging to tty/file/clipboard
    IMGUI_API void          LogText(const char* fmt, ...) IM_FMTARGS(1);                        // pass text data straight to log (without being displayed)

    // Drag and Drop
    // - If you stop calling BeginDragDropSource() the payload is preserved however it won't have a preview tooltip (we currently ui a fallback "..." tooltip as replacement)
    ...
    IMGUI_API const char*   GetStyleColorName(ImGuiCol idx);                                    // get a string corresponding to the enum value (for ui, saving, etc.).
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_Password            = 1 << 15,  // Password mode, ui all characters as '*'
...
enum ImGuiSelectableFlags_
{
    ...
    ImGuiSelectableFlags_Disabled           = 1 << 3,   // Cannot be selected, ui grayed out text
...
enum ImGuiTableFlags_
{
    ...
    ImGuiTableFlags_Reorderable                = 1 << 1,   // Enable reordering columns in header row (need calling TableSetupColumn() + TableHeadersRow() to ui headers)
    ImGuiTableFlags_Hideable                   = 1 << 2,   // Enable hiding/disabling columns in context menu.
    ImGuiTableFlags_Sortable                   = 1 << 3,   // Enable sorting. Call TableGetSortSpecs() to obtain sort specs. Also see ImGuiTableFlags_SortMulti and ImGuiTableFlags_SortTristate.
    ImGuiTableFlags_NoSavedSettings            = 1 << 4,   // Disable persisting columns order, width and sort settings in the .ini file.
    ImGuiTableFlags_ContextMenuInBody          = 1 << 5,   // Right-click on columns body/contents will ui table context menu. By default it is available in TableHeadersRow().
    // Decorations
...
enum ImGuiDockNodeFlags_
{
    ...
    ImGuiDockNodeFlags_KeepAliveOnly                = 1 << 0,   // Shared       // Don't ui the dockspace node but keep it alive. Windows docked into this dockspace node won't be undocked.
    ...
    ImGuiDockNodeFlags_PassthruCentralNode          = 1 << 3,   // Shared       // Enable passthru dockspace: 1) DockSpace() will render a ImGuiCol_WindowBg background covering everything excepted the Central Node when empty. Meaning the host window should probably use SetNextWindowBgAlpha(0.0f) prior to Begin() when using this. 2) When Central Node is empty: let inputs pass-through + won't ui a DockingEmptyBg background. See demo for details.
...
enum ImGuiDragDropFlags_
{
    ...
    ImGuiDragDropFlags_SourceNoPreviewTooltip       = 1 << 0,   // By default, a successful call to BeginDragDropSource opens a tooltip so you can ui a preview or description of the source contents. This flag disable this behavior.
...
enum ImGuiColorEditFlags_
{
    ...
    ImGuiColorEditFlags_NoLabel         = 1 << 7,   //              // ColorEdit, ColorPicker: disable ui of inline text label (the label is still forwarded to the tooltip and picker).
    ...
    ImGuiColorEditFlags_AlphaPreview    = 1 << 17,  //              // ColorEdit, ColorPicker, ColorButton: ui preview as a transparent color over a checkerboard, instead of opaque.
    ImGuiColorEditFlags_AlphaPreviewHalf= 1 << 18,  //              // ColorEdit, ColorPicker, ColorButton: ui half opaque / half checkerboard, instead of opaque.
...
enum ImGuiSliderFlags_
{
    ...
    ImGuiSliderFlags_NoRoundToFormat        = 1 << 6,       // Disable rounding underlying value to match precision of the ui format string (e.g. %.3f values are rounded to those 3 digits)
...
// User code may request backend to ui given cursor by calling SetMouseCursor(), which is why we have some cursors that are marked unused here
...
struct ImGuiStyle
{
    ...
    ImVec2      DisplayWindowPadding;       // Window position are clamped to be visible within the ui area or monitors by at least this amount. Only applies to regular windows.
...
struct ImGuiIO
{
    ...
    ImVec2      DisplaySize;                    // <unset>          // Main ui size, in pixels (generally == GetMainViewport()->Size)
    ...
    ImVec2      DisplayFramebufferScale;        // = (1, 1)         // For retina ui or other situations where window coordinates are different from framebuffer coordinates. This generally ends up in ImDrawData::FramebufferScale.

    // Docking options (when ImGuiConfigFlags_DockingEnable is set)
    bool        ConfigDockingNoSplit;           // = false          // Simplified docking mode: disable window splitting, so docking is limited to merging multiple windows together into tab-bars.
    bool        ConfigDockingWithShift;         // = false          // Enable docking with holding Shift key (reduce visual noise, allows dropping in wider space)
    bool        ConfigDockingAlwaysTabBar;      // = false          // [BETA] [FIXME: This currently creates regression with auto-sizing and general overhead] Make every single floating window ui within a docking node.
    bool        ConfigDockingTransparentPayload;// = false          // [BETA] Make window or viewport transparent when docking and only ui docking boxes on the target viewport. Useful if rendering of multiple viewport cannot be synced. Best used with ConfigViewportsNoAutoMerge.
    ...
    bool        WantTextInput;                  // Mobile/console: when set, you may ui an on-screen keyboard. This is set by Dear ImGui when it wants textual keyboard input to happen (e.g. when a InputText widget is active).
...
// - Clipper calculate the actual range of elements to ui based on the current clipping rectangle, position the cursor before the first visible element.
...
struct ImDrawList
{
    ...
    // - "uv_min" and "uv_max" represent the normalized texture coordinates to use for those corners. Using (0,0)->(1,1) texture coordinates will generally ui the entire texture.
...
struct ImDrawData
{
    ...
    ImVec2          FramebufferScale;       // Amount of pixels for each unit of DisplaySize. Based on io.DisplayFramebufferScale. Generally (1,1) on normal ui, (2,2) on OSX with Retina ui.
...
struct ImFontAtlasCustomRect
{
    ...
    ImVec2          GlyphOffset;    // Input    // For custom font glyphs only: glyph ui offset
...
// (Optional) This is required when enabling multi-viewport. Represent the bounds of each connected monitor/ui and their DPI.
