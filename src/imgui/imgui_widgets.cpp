...
// Even though we changed the compile-time default, we expect users to have carried %f around, which would break the ui of DragInt() calls.
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // Display value using user-provided ui format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
...
// Parse ui precision back from the ui format string
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (is_multiline)
    {
        ...
        draw_window->DC.NavLayerActiveMaskNext |= (1 << draw_window->DC.NavLayerCurrent); // This is to ensure that EndChild() will ui a navigation highlight so we can "enter" into it.
    ...
    if ((init_state && g.ActiveId != id) || init_changed_specs)
    {
        ...
        state->TextAIsValid = false;                // TextA is not valid yet (we will ui buf until then)
    ...
    if (render_cursor || render_selection)
    {
        ...
        // - Handle scrolling, highlight selection, ui cursor (those all requires some form of 1d->2d cursor position calculation)
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    // Context menu: ui and modify options (before defaults are applied)
    ...
    if ((flags & (ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV)) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        {
            {   "%3d",   "%3d",   "%3d",   "%3d" }, // Short ui
            { "R:%3d", "G:%3d", "B:%3d", "A:%3d" }, // Long ui for RGBA
            { "H:%3d", "S:%3d", "V:%3d", "A:%3d" }  // Long ui for HSVA
        };
        static const char* fmt_table_float[3][4] =
        {
            {   "%0.3f",   "%0.3f",   "%0.3f",   "%0.3f" }, // Short ui
            { "R:%0.3f", "G:%0.3f", "B:%0.3f", "A:%0.3f" }, // Long ui for RGBA
            { "H:%0.3f", "S:%0.3f", "V:%0.3f", "A:%0.3f" }  // Long ui for HSVA
        };
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    // Context menu: ui and store options.
    if (!(flags & ImGuiColorEditFlags_NoOptions))
    ...
    if (value_changed)
    {
        if (flags & ImGuiColorEditFlags_InputRGB)
        {
            ...
            B = col[2];
            ColorConvertRGBtoHSV(R, G, B, H, S, V);
            if (memcmp(g.ColorEditLastColor, col, sizeof(float) * 3) == 0) // Fix local Hue as ui below will use it immediately.
            {
                if (S == 0)
                    H = g.ColorEditLastHue;
                if (V == 0)
...
// FIXME: May want to ui/ignore the alpha component in the color ui? Yet show it in the tooltip.
// 'desc_id' is not called 'label' because we don't ui it next to the button, but only in the tooltip.
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    {
        // Because GetColorU32() multiplies by the global style Alpha and we don't want to ui a checkerboard if the source code had no alpha
...
bool ImGui::BeginMenuBar()
{
    ...
    // We remove 1 worth of rounding to Max.x to that text in long menus and small windows don't tend to ui over the lower-right rounded area, which looks particularly glitchy.
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    {
        if (menu_is_open)
            menu_is_open = BeginPopupEx(id, flags); // menu_is_open can be 'false' when the popup is completely clipped (e.g. zero size ui)
        else
