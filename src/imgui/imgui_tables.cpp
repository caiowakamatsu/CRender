...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        if (column->Flags & ImGuiTableColumnFlags_WidthFixed)
        {
            ...
            // large height (= first frame scrollbar ui very off + clipper would skip lots of items).
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        // - ClipRect.Max.x: using MaxX makes it easier for header to receive hover highlight with no discontinuity and ui sorting arrow.
    ...
    // [Part 13] Sanitize and build sort specs before we have a change to use them for ui.
...
// - We allocate them following storage order instead of ui order so reordering columns won't needlessly
...
void ImGui::TableDrawBorders(ImGuiTable* table)
{
    ...
    {
        // Display outer border offset by 1 which is a simple way to ui it without adding an extra draw call
        // (Without the offset, in outer_window it would be rendered behind cells, because child windows are above their
        // parent. In inner_window, it won't reach out over scrollbars. Another weird solution would be to ui part
...
// Note that because of how we cpu-clip and ui sorting indicators, you _cannot_ use SameLine() after a TableHeader()
...
void ImGui::TableLoadSettings(ImGuiTable* table)
{
    ...
    // Validate and fix invalid ui order data
