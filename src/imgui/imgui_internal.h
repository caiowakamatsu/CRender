...
#endif
#define IM_TABSIZE                      (4)
#define IM_F32_TO_INT8_UNBOUND(_VAL)    ((int)((_VAL) * 255.0f + ((_VAL)>=0 ? 0.5f : -0.5f)))   // Unsaturated, for ui purpose
...
struct IMGUI_API ImRect
{
    ...
    void        ClipWith(const ImRect& r)           { Min = ImMax(Min, r.Min); Max = ImMin(Max, r.Max); }                   // Simple version, may lead to an inverted rectangle, which is fine for Contains/Overlaps test but not for ui.
...
struct ImGuiContext
{
    ...
    // Windows state
    ImVector<ImGuiWindow*>  Windows;                            // Windows, sorted in ui order, back to front
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    ImRect                  LastItemDisplayRect;    // End-user ui rect for last item (only valid if LastItemStatusFlags & ImGuiItemStatusFlags_HasDisplayRect)
...
struct IMGUI_API ImGuiWindow
{
    ...
    bool                    Hidden;                             // Do not ui (== HiddenFrames*** > 0)
    ...
    ImGuiWindow*            RootWindowForTitleBarHighlight;     // Point to ourself or first ancestor which will ui TitleBgActive color when this window is active.
...
struct ImGuiTable
{
    ...
    ImSpan<ImGuiTableColumnIdx> DisplayOrderToIndex;        // Point within RawData[]. Store ui order of columns (when not reordered, the values are 0...Count-1)
    ...
    bool                        IsDefaultDisplayOrder;      // Set when ui order is unchanged from default (DisplayOrder contains 0...Count-1)
